...
func (kv *ShardKV) Get(args *GetArgs, reply *GetReply) {
	...
	if in := kv.checkKeyInGroup(args.Key); !in {
		...
		DPrintf("Group[%d]ShardKV[%d] respond Get(key:%v) ownstate %v ErrWrongGroup\n", kv.gid, kv.me, args.Key, kv.ownedShards)
		kv.mu.Unlock()
		return
	}
	kv.mu.Unlock()
...
func (kv *ShardKV) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	...
	// ShardKV过滤重复请求
	if int(args.CmdId) < kv.sessions[args.ClientId].LastCmdNum {
		// 标志这个请求client已经收到正确的回复了，只是这个重发请求到达得太慢了
		// 其实client端并没有在等待这个回复了，直接return
		kv.mu.Unlock()
		return
	} else if int(args.CmdId) == kv.sessions[args.ClientId].LastCmdNum {
...
func (kv *ShardKV) checkAndGetShard() {
	for !kv.Killed() {
		...
		kv.mu.Unlock()
		DPrintf("Group[%d]ShardKV[%d] waitGetShards %v\n", kv.gid, kv.me, waitGetShards)
...
func (kv *ShardKV) checkAndFinishGiveShard() {
	for !kv.Killed() {
		...
		curConfig := kv.curConfig
		kv.mu.Unlock()
		DPrintf("Group[%d]ShardKV[%d] waitGiveShards %v\n", kv.gid, kv.me, waitGiveShards)
...
func (kv *ShardKV) addEmptyLog() {
	for !kv.Killed() {
		...
		if !kv.rf.HaveCurrentTermLog() { // 返回false代表没有当前term的日志条目
			emptyOp := Op{
				OpType: EmptyOp,
			}
